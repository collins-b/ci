...
if [ -z "$USE_CIRCLECI_BETA" ]; then
  ...
  sudo /home/circleci/google-cloud-sdk/bin/gcloud --quiet version
  sudo /home/circleci/google-cloud-sdk/bin/gcloud --quiet components update --version 120.0.0
  sudo /home/circleci/google-cloud-sdk/bin/gcloud --quiet components update --version 120.0.0 kubectl
fi

# set key and authenticate gcloud
echo $GCLOUD_SERVICE_KEY | base64 --decode > ${HOME}/gcloud-service-key.json
sudo /home/circleci/google-cloud-sdk/bin/gcloud auth activate-service-account --key-file ${HOME}/gcloud-service-key.json

# configure gcloud
sudo /home/circleci/google-cloud-sdk/bin/gcloud --quiet config set project $PROJECT_NAME
sudo /home/circleci/google-cloud-sdk/bin/gcloud --quiet config set container/cluster $CLUSTER_NAME
sudo /home/circleci/google-cloud-sdk/bin/gcloud --quiet config set compute/zone ${CLOUDSDK_COMPUTE_ZONE}
sudo /home/circleci/google-cloud-sdk/bin/gcloud --quiet container clusters get-credentials $CLUSTER_NAME
...
if [ -z "$USE_CIRCLECI_BETA" ]; then
  echo " Building image"
  sudo /home/circleci/google-cloud-sdk/bin/gcloud docker -- build -t gcr.io/${PROJECT_NAME}/${IMAGE}:$IMG_TAG  ${DOCKER_SOURCE} > /dev/null
  sudo docker tag -f gcr.io/${PROJECT_NAME}/${IMAGE}:${IMG_TAG} gcr.io/${PROJECT_NAME}/${IMAGE}:latest
  echo " Successfully built"

  echo " Pushing image"
  sudo /home/circleci/google-cloud-sdk/bin/gcloud docker push gcr.io/${PROJECT_NAME}/${IMAGE}:$IMG_TAG
  sudo /home/circleci/google-cloud-sdk/bin/gcloud docker push gcr.io/${PROJECT_NAME}/${IMAGE}:latest
